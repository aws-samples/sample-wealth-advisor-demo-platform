...
resource "aws_iam_role_policy" "lambda_permissions" {
  ...
  #checkov:skip=CKV_AWS_290: "Ensure IAM policies does not allow write access without constraints": "sso:CreateInstance requires Resource '*'. Action already tightened to minimum required."
  #checkov:skip=CKV_AWS_289: "Ensure IAM policies does not allow permissions management / resource exposure without constraints": "sso:CreateInstance requires Resource '*'. Action already tightened to minimum required."
}
