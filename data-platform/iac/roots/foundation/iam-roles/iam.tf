...
resource "aws_iam_role_policy" "glue_policy" {
  ...
  # CKV_AWS_355: Multiple actions require Resource = "*" (lakeformation, datazone, cloudtrail, kafka list operations).
  # Additionally, kafka-cluster:* and kafka:* wildcard statements duplicate the scoped statements below them.
  # TODO: Remove duplicate wildcard kafka/kafka-cluster statements and rely on the resource-scoped ones.
  #checkov:skip=CKV_AWS_355: "Ensure no IAM policies documents allow '*' as a statement's resource for restrictable actions": "Several actions (lakeformation, datazone, cloudtrail) do not support resource-level permissions. Kafka actions have scoped duplicates."
  #checkov:skip=CKV_AWS_290: "Ensure IAM policies does not allow write access without constraints": "lakeformation, datazone, and kafka write actions require Resource '*'. TODO: Remove duplicate wildcard kafka statements."
}
...
resource "aws_iam_role_policy" "lakeformation_service_role_policy" {
  ...
  # WARNING: no-iam-resource-exposure — lakeformation:PutDataLakeSettings and lakeformation:RegisterResource
  # are resource-exposure actions on Resource = "*" (Lake Formation does not support resource-level permissions).
  # PutDataLakeSettings can modify data lake admins. RegisterResource controls which S3 locations Lake Formation governs.
  # This role is only assumable by lakeformation.amazonaws.com, glue.amazonaws.com, and athena.amazonaws.com.
  # TODO: Split this statement by service with correct resource ARNs. The current statement mixes s3tables, glue,
  # lakeformation, and s3 actions under a single s3tables resource ARN, which may cause lakeformation/glue actions
  # to be ineffective. Consider removing PutDataLakeSettings if not required for ongoing operations.
}
...
resource "aws_iam_policy" "domain_execution_policy_ram" {
  ...
  # WARNING: no-iam-resource-exposure — ram:CreateResourceShare, ram:AssociateResourceShare, and other RAM
  # actions can share AWS resources with other accounts. RAM does not support resource-level IAM permissions
  # for most actions. DataZone requires these to share resources across projects/environments. This role is
  # only assumable by datazone.amazonaws.com and cloudformation.amazonaws.com (with source account condition).
}
...
resource "aws_iam_policy" "domain_execution_policy_kms" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid = "KMSGrantAndReEncryptActions"
        Action = [
          "kms:CreateGrant",
          "kms:RevokeGrant",
          "kms:ReEncryptTo"
...
resource "aws_iam_policy" "quicksight_custom_service_policy" {
  ...
  # WARNING: no-iam-resource-exposure — lakeformation:GrantPermissions and lakeformation:BatchGrantPermissions
  # can grant other principals access to Lake Formation-managed data. Lake Formation does not enforce IAM
  # resource-level constraints for grant operations. This role is only assumable by quicksight.amazonaws.com
  # (with source account condition). QuickSight needs these actions to delegate data access when users create
  # datasets. Consider removing GrantPermissions/BatchGrantPermissions if QuickSight only reads data.
}
