...
export interface ThemeSchedulerGenerateGeneralThemesProps {
  readonly redshiftWorkgroup: string;
  readonly redshiftDatabase: string;
  readonly webCrawlerMcpArn: string;
}

export class ThemeSchedulerGenerateGeneralThemes extends Function {
  constructor(
    scope: Construct,
    id: string,
    props: ThemeSchedulerGenerateGeneralThemesProps,
  ) {
    super(scope, id, {
      ...
      handler:
        'wealth_management_portal_scheduler_tools.lambda_functions.generate_general_themes.lambda_handler',
      code: Code.fromAsset(
        url.fileURLToPath(
          new URL(
